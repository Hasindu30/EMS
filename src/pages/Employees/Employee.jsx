...
const Employee = () => {
  ...
  const fetchEmployees = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/allemployee',{
        params: { search },
      });
      setData(response.data);
      generateNextEmployeeCode(response.data); 
      setLoading(false);
    } catch (error) {
      console.error(error);
      setLoading(false);
  ...
  const generateNextEmployeeCode = (employees) => {
    ...
    const lastCode = lastEmployee.empCode || 'EMP000'; 
    ...
    const handleDelete = (id) => {
      ...
      const confirmDelete = async () => {
        try {
          ...
          await axios.delete(`/api/employeeDelete/${deleteEmployeeId}`);
          ...
          setDeleteEmployeeId(null); // clear selected id
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (isEditMode) {
        ...
        await axios.put(`/api/employeeUpdate/${selectedEmployeeId}`, employeeData);
      ...
      console.error(error);
      toast.error('Failed to create or update employee!', {
        position: "top-center",
        autoClose: false,
      });
    }
    ...
    <div className="flex min-h-screen bg-gray-100 text-gray-800">
      ...
      <div className={`flex-1 transition-all duration-300 ${isHovered ? 'ml-64' : 'ml-20'}`}>
        ...
        <div className="p-6 mt-10">
          <h1 className="text-2xl font-semibold mb-3">Employee Registry</h1>
